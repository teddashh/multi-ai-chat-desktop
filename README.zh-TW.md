...
- **可自訂協作角色。** 內建四角色預設會讓四家 provider 各擔任一次；你仍可在「設定」重新指定角色。依序執行的角色可重複使用同一家，同時執行的角色則必須分開。
...
| **道理辯證** | 5 輪 × 4 席 = 20 次發言；預設四家 provider 各一席 | 對困難題目慢慢攻防、收斂 |
| **腦力激盪** | 12 輪 × 4 個輪換席位 = 48 次發言；問題框定 → 發散 → 交叉激發 → 分群選擇 → 概念驗證 | 帶完整前文的點子深化、平衡方案組合與首輪實驗 |

結構化 workflow 遇到 provider 網頁拒絕剛結束後的連續送出時，會自動重試一次；預設情況下，若仍失敗或遇到其他引擎錯誤，流程會立即停止，不再把錯誤字串當成回答傳給後續角色。腦力激盪則會暫停並提供「重試／略過／取消」；選擇「略過」會記錄安全的佔位內容，再繼續下一席，不會轉傳原始錯誤文字。

腦力激盪刻意設計成最重的預設：請先登入預設的四家網頁 session，並預留約 45–90 分鐘。若有 provider 無法使用，啟動前檢查會清楚指出，你也可以自訂角色分配。Claude 目前的消費者網站要求帳號登入；本 app 只會辨識並引導官方登入流程，不會繞過 provider 的登入或安全驗證。

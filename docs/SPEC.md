...
| loggedOutDetectors | `[data-testid="login-button"]` · `[data-testid="signup-button"]` · `[data-testid="login-form"]` | `input[type="email"]` · `input[autocomplete="email"]` | empty | localized `button` text detectors for sign-in/sign-up in English, Traditional/Simplified Chinese, Japanese, and German |

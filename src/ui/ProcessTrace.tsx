...
export function ProcessTrace({ trace, locale = 'en' }: { trace: ProcessTraceState; locale?: Locale }) {
  ...
  return (
    <section aria-label={t('processTrace.title', locale)} className="mt-3 border border-zinc-800 bg-zinc-900">
      <div className="flex items-start justify-between gap-3 border-b border-zinc-800 px-3 py-2">
        <h2 className="text-xs font-semibold uppercase text-zinc-300">{t('processTrace.title', locale)}</h2>
        <div className="min-w-0 text-right text-[11px] text-sky-200">{trace.currentStatus || t('processTrace.settled', locale)}</div>
      </div>
      {trace.steps.length > 0 ? (
        ...
        <div className="px-3 py-2 text-xs text-zinc-500">{t('processTrace.waitingEvents', locale)}</div>
      )}
    </section>
  );
}

...
export function RoleConfig({
  ...
  return (
    <div className="mt-3 space-y-2 border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 p-3">
      {ROLE_KEYS[mode].map((roleKey) => (
        <div key={roleKey} className="flex items-start gap-2">
          <span className="w-20 flex-none pt-1 text-xs text-zinc-700 dark:text-zinc-300">{ROLE_LABELS[mode][roleKey]}</span>
            ...
            {PROVIDERS.map((provider) => {
              ...
              return (
                <button
                  ...
                  className={`px-2 py-1 text-center text-xs transition ${selected ? 'font-bold' : 'text-zinc-500 dark:text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200'}`}
